...
import { useCheckout } from "~/vertex/components/checkout/checkout-context"
...
export default function PaymentSection({ ...props }: PaymentSectionProps) {
  ...
  const { paymentMethodProps, data, paymentMethod } = useCheckout()
      ...
      <RadioGroup {...paymentMethodProps}>
        {paymentOptions.map((a) => (
          <label
            key={a.type}
            htmlFor={a.type}
            className={cn("flex cursor-pointer items-center space-x-4 rounded-md border bg-white p-4 py-4 shadow", {
              "border-primary": paymentMethod === a.type,
              "cursor-not-allowed opacity-80": !a.isEligible,
            })}
          >
            <RadioGroupItem value={a.type} id={a.type} disabled={!a.isEligible} />
