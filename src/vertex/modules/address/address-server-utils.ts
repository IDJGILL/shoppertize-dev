...
export const convertAddress = (props: { id?: string; address: Address; notificationEmail: string }) => {
  return {
    id: props.id ?? nanoid(),
    address: {
      shipping: {
        ...
        state: findState({ type: "name", return: "code", value: props.address.shipping_state }),
      ...
      billing: props.address.isTaxInvoice
        ? {
            ...
            state: findState({ type: "name", return: "code", value: props.address.billing_state }),
...
export const getCurrentAddressFromSession = async (uid: string) => {
